import { z } from 'zod';
export const kinds = ['companies', 'contacts', 'deals', 'tasks', 'activities', 'notes', 'products', 'proformas', 'automations', 'stock_movements'] as const;
...
export const activityTypes = ['call', 'meeting', 'task', 'email', 'sms', 'whatsapp'] as const;
export type ActivityType = typeof activityTypes[number];
export const activityTypeLabels: Record<ActivityType, string> = { call: 'تماس', meeting: 'جلسه', task: 'وظیفه', email: 'ایمیل', sms: 'پیامک', whatsapp: 'واتساپ' };
export const activityDirections = ['none', 'outbound', 'inbound'] as const;
export const activityDirectionLabels: Record<string, string> = { none: 'بدون جهت', outbound: 'خروجی', inbound: 'ورودی' };
export const labels: Record<Kind, string> = { companies: 'مشتریان', contacts: 'مخاطبان', deals: 'فرصت‌های فروش', tasks: 'پیگیری‌ها', activities: 'فعالیت‌ها', notes: 'یادداشت‌ها', products: 'محصولات', proformas: 'پیش‌فاکتورها', automations: 'اتوماسیون فروش', stock_movements: 'گردش انبار' };
...
export function validateSales(kind: Kind, data: Data) {
    ...
    if (kind === 'activities' && !['open', 'done'].includes(data.status)) return 'وضعیت فعالیت معتبر نیست.';
    if (kind === 'activities' && data.activity_time && data.activity_end_time && data.activity_end_time < data.activity_time) return 'ساعت پایان نمی‌تواند قبل از ساعت شروع باشد.';
...
export function summary(rows: Row[]) { const deals = rows.filter(r => r.kind === 'deals'); const won = deals.filter(r => r.data.stage === 'won'); const lost = deals.filter(r => r.data.stage === 'lost'); return { customers: rows.filter(r => r.kind === 'companies').length, pipeline: deals.filter(r => !['won', 'lost'].includes(r.data.stage)).reduce((s, r) => s + r.data.amount, 0), revenue: won.reduce((s, r) => s + r.data.amount, 0), rate: won.length + lost.length ? Math.round(won.length / (won.length + lost.length) * 100) : 0, openTasks: rows.filter(r => ['tasks','activities'].includes(r.kind) && r.data.status !== 'done').length }; }
