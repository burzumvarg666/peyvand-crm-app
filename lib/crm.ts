...
export const salesSchema = z.object({
    ...
    quote_number: z.string().trim().max(80).default(''), quote_status: z.enum(quoteStatuses).default('draft'), valid_until: date.default(''),
...
export function exportCsv(rows: Row[]) { return '\ufeff' + [['ردیف', 'شناسه', 'نوع', 'عنوان', 'ایمیل', 'تلفن', 'شهر', 'مرحله', 'مبلغ (تومان)', 'موعد (میلادی)', 'وضعیت', 'توضیحات','کد محصول','واحد','قیمت واحد','موجودی','حداقل موجودی','شماره پیش‌فاکتور','مبلغ نهایی پیش‌فاکتور','نوع گردش','تغییر موجودی','مانده انبار','مرجع'], ...rows.map((r,index) => [index+1,r.id, labels[r.kind], r.data.name, r.data.email, r.data.phone, r.data.city, r.kind==='deals'?stageLabels[r.data.stage]:'',r.kind==='deals'?r.data.amount:'',r.kind==='proformas'?r.data.valid_until:r.data.due,r.kind==='proformas'?quoteStatusLabels[r.data.quote_status]:statusLabels[r.data.status],r.data.description,r.kind==='products'?r.data.sku:'',['products','stock_movements'].includes(r.kind)?r.data.unit:'',r.kind==='products'?r.data.price:'',r.kind==='products'?r.data.stock:'',r.kind==='products'?r.data.min_stock:'',r.kind==='proformas'?r.data.quote_number:'',r.kind==='proformas'?quoteAmounts(r.data).total:'',r.kind==='stock_movements'?r.data.movement_type:'',r.kind==='stock_movements'?r.data.movement_quantity:'',r.kind==='stock_movements'?r.data.stock_after:'',r.kind==='stock_movements'?r.data.movement_reference:''])].map(r => r.map(csvCell).join(',')).join('\r\n'); }
