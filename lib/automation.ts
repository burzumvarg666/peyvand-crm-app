...
export function automationCandidates(rows: Row[], day: string, change?: AutomationChange) {
    ...
    const candidates: {kind:'activities'|'notes';data:Data;parent_id:string|null}[]=[];
    ...
    for(const rule of rules){
        ...
        if(rule.data.automation_trigger==='stage_change'){
            ...
            sources=rows.filter(r=>(r.kind==='tasks'||(r.kind==='activities'&&r.data.activity_type==='task'))&&r.data.status==='open'&&!r.data.automation_key&&r.data.due&&r.data.due<day);
        }
        for(const source of sources){
            ...
            candidates.push({kind:notify?'notes':'activities',parent_id:notify?source.id:source.parent_id,data:{...blank(),name,status:'open',activity_type:'task',due:rule.data.automation_trigger==='quote_expiry'?day:addDays(day,rule.data.automation_days),description:rule.data.description||`اجرای قانون «${rule.data.name}»`,automation_key:storedKey}});
        }
    }
    return candidates;
}
