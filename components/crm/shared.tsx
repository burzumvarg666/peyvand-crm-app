...
export function PersianDateInput({value,onChange,ariaLabel='تاریخ'}:{value:string;onChange:(value:string)=>void;ariaLabel?:string}){
 ...
 <div className="flex items-center justify-between mb-3"><Button type="button" variant="ghost" size="icon" aria-label="ماه قبل" onClick={()=>setMonth(new Date(first.getTime()-86400000))}><ChevronRight size={18}/></Button><div className="row"><select aria-label="انتخاب ماه شمسی" value={Number(jalaliParts(month).month)} onChange={e=>{const iso=fromPersianDate(jalaliParts(month).year+'/'+e.target.value+'/01');if(iso)setMonth(new Date(iso+'T12:00:00Z'));}}>{['فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور','مهر','آبان','آذر','دی','بهمن','اسفند'].map((name,i)=><option key={name} value={i+1}>{name}</option>)}</select><select aria-label="انتخاب سال شمسی" value={Number(jalaliParts(month).year)} onChange={e=>{const iso=fromPersianDate(e.target.value+'/'+jalaliParts(month).month+'/01');if(iso)setMonth(new Date(iso+'T12:00:00Z'));}}>{Array.from({length:201},(_,i)=>Number(jalaliParts(new Date()).year)-100+i).map(y=><option key={y} value={y}>{num(y).replace(/٬/g,'')}</option>)}</select></div><Button type="button" variant="ghost" size="icon" aria-label="ماه بعد" onClick={()=>setMonth(nextFirst)}><ChevronLeft size={18}/></Button></div>
...
export function Pick({value,onChange,options,label}:{value:string;onChange:(s:string)=>void;options:{value:string;label:string}[];label:string}){
 return <Select dir="rtl" value={value||'none'} onValueChange={v=>onChange(v==='none'?'':v)}><SelectTrigger className="pick" aria-label={label}><SelectValue placeholder={label}/></SelectTrigger><SelectContent position="popper" align="start" sideOffset={4} className="crm-pick-menu">{options.map(o=><SelectItem value={o.value||'none'} key={o.value||'none'}>{o.label}</SelectItem>)}</SelectContent></Select>;
}
