...
export function Detail({row,rows,members,canEdit,busy,desktop,onClose,onEdit,onDelete,onSave,onOpen,onExcel,onPdf}:{row:Row;rows:Row[];members:State['members'];canEdit:boolean;busy:boolean;desktop:boolean;onClose:()=>void;onEdit:()=>void;onDelete:(r:Row)=>void;onSave:(kind:Row['kind'],data:Data,parent:string|null,record?:Row)=>Promise<boolean>;onOpen:(id:string)=>void;onExcel:()=>void;onPdf:()=>void}){
 ...
 else if(row.kind==='proformas')fields.push(['شماره',d.quote_number||'—'],['پایان اعتبار',date(d.valid_until)],['وضعیت',quoteStatusLabels[d.quote_status]]);
 ...
 else if(row.kind==='tasks')fields.push(['نوع فعالیت','وظیفه'],['موعد',date(d.due)],['وضعیت',statusLabels[d.status]]);
 else if(row.kind==='activities')fields.push(['نوع فعالیت',activityTypeLabels[d.activity_type]],['جهت',activityDirectionLabels[d.activity_direction]],['تاریخ',date(d.due)],['ساعت شروع',d.activity_time||'—'],['ساعت پایان',d.activity_end_time||'—'],['محل / لینک',d.activity_location||'—'],['نتیجه',d.activity_result||'—'],['وضعیت',statusLabels[d.status]]);
 if(row.kind==='companies'&&d.status!=='lead'&&d.start_date)fields.push(['تاریخ شروع همکاری',date(d.start_date)]);
 if(d.email)fields.push(['ایمیل',d.email]);if(d.phone)fields.push(['تلفن',d.phone]);if(d.city)fields.push(['شهر',d.city]);if(d.website)fields.push(['وب‌سایت',d.website]);if(d.address)fields.push(['آدرس',d.address]);if(d.lead_source)fields.push(['منبع سرنخ',d.lead_source]);if(row.kind==='companies'&&d.employee_count)fields.push(['تعداد پرسنل',num(d.employee_count)]);if(row.kind==='deals'){fields.push(['احتمال فروش',num(d.probability)+'٪'],['تاریخ پیش‌بینی فروش',date(d.estimated_sales_date)],['کمپین موثر',d.campaign||'—'],['قدم بعدی',d.next_step||'—']);}if(row.kind==='proformas'){fields.push(['وضعیت پرداخت',d.payment_status==='paid'?'تسویه‌شده':d.payment_status==='partial'?'تسویهٔ ناقص':'پرداخت‌نشده'],['روش تسویه',d.payment_method||'—'],['نماینده خریدار',d.buyer_representative||'—'],['کد اقتصادی',d.economic_code||'—']);}if(d.assignee)fields.push(['مسئول',members.find(m=>m.user_id===d.assignee)?.email||'تعیین نشده']);
