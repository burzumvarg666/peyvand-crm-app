...
export class CRMStore{
 ...
 delete(input){const p=z.object({id:uuid,version:z.number().int().positive()}).safeParse(input);if(!p.success)throw new StoreError('شناسه معتبر نیست.');return this.transaction(()=>{const old=this.db.prepare('SELECT * FROM records WHERE id=?').get(p.data.id);if(!old||old.version!==p.data.version)throw new StoreError('اطلاعات تغییر کرده است. صفحه را تازه کنید.',409);if(old.kind==='stock_movements')throw new StoreError('سند انبار حذف نمی‌شود؛ یک اصلاح موجودی ثبت کنید.');if(old.kind==='products'&&(JSON.parse(old.data).stock>0||this.records().some(r=>r.kind==='stock_movements'&&r.parent_id===old.id)))throw new StoreError('محصول دارای سابقهٔ انبار است؛ وضعیت آن را غیرفعال کنید.');const children=this.db.prepare('SELECT id,data FROM records WHERE parent_id=?').all(old.id);this.db.prepare('UPDATE records SET parent_id=NULL,version=version+1,updated_at=? WHERE parent_id=?').run(new Date().toISOString(),old.id);for(const child of children)this.log(child.id,'UPDATE',JSON.parse(child.data).name);this.db.prepare('DELETE FROM records WHERE id=?').run(old.id);this.log(old.id,'DELETE',JSON.parse(old.data).name);});}
